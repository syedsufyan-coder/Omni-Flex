...
function submitCreateSection() {
    ...
    const payload = {
        ...
        department: document.getElementById('create_department').value,
        degree: document.getElementById('create_degree').value,
        batch: parseInt(document.getElementById('create_batch').value) || 0
    };

    if (!payload.sectionId) return showAlert('createSectionError', 'Section ID is required');
    if (!payload.sectionLabel) return showAlert('createSectionError', 'Section Label is required');
    if (!payload.department) return showAlert('createSectionError', 'Department is required');
    if (!payload.degree) return showAlert('createSectionError', 'Degree is required');
    if (!payload.batch || payload.batch < 2020)
        return showAlert('createSectionError', 'Valid Batch Year is required (min 2020)');
...
function openViewSectionModal(sectionId) {
    ...
    $.ajax({
        ...
        success: function (res) {
            ...
            const barColor = seatsBar >= 90 ? 'bg-danger' : seatsBar >= 70 ? 'bg-warning' : 'bg-success';
...
function submitEditSection() {
    ...
    const payload = {
        ...
        department: document.getElementById('edit_new_department').value,
        degree: document.getElementById('edit_new_degree').value,
        batch: parseInt(document.getElementById('edit_new_batch').value) || null
    };

    if (!payload.sectionLabel) return showAlert('editSectionError', 'Section Label is required');
    if (!payload.department) return showAlert('editSectionError', 'Department is required');
    if (!payload.degree) return showAlert('editSectionError', 'Degree is required');
...
function openAssignCRModal(sectionId) {
    ...
    document.getElementById('assign_studentId').innerHTML = '<option value="">Loading...</option>';
    document.getElementById('currentCR').innerHTML = '<p class="text-muted small">Loading...</p>';
...
function submitAssignCR() {
    ...
    if (!payload.studentId) return showAlert('assignCRError', 'Please select a student');
    ...
    $.ajax({
        ...
        success: function (res) {
            if (res.success) {
                ...
                setTimeout(() => {
                    bootstrap.Modal.getInstance(document.getElementById('assignCRModal')).hide();
                    location.reload();
                }, 1000);
            } else {
...
function openDeleteSectionModal(sectionId, displayName) {
    ...
    deleteBtn.innerHTML = '<span class="spinner-border spinner-border-sm me-1"></span>Checking...';
    ...
    $.ajax({
        ...
        success: function (res) {
            if (res.success && res.data && res.data.length > 0) {
                showAlert('deleteSectionError',
                    `Cannot delete: ${res.data.length} student(s) enrolled. Remove enrollments first.`);
                deleteBtn.disabled = true;
                deleteBtn.innerHTML = '<i class="bi bi-exclamation-triangle me-1"></i>Cannot Delete';
            } else {
    ...
    $.ajax({
        ...
        success: function (res) {
            if (res.success) {
                ...
                const row = document.querySelector(`#sectionsTable tr[data-section-id="${sectionId}"]`);
                if (row) {
