...
function submitAddUser() {
  ...
  if (role === "Student") {
    payload.batch =
      parseInt(document.getElementById("add_batch").value) || null;
  ...
  if (role === "Instructor") {
    ...
    payload.specialization = document
      .getElementById("add_specialization")
      .value.trim();
  }
  if (role === "Admin") {
    payload.designation = document
      .getElementById("add_adminDesignation")
      .value.trim();
    payload.officeRoom = document
      .getElementById("add_adminOfficeRoom")
      .value.trim();
  }
  ...
  if (!payload.firstName)
    return showAlert("addUserError", "First Name is required");
  if (!payload.lastName)
    return showAlert("addUserError", "Last Name is required");
  if (!payload.email || !payload.email.includes("@"))
    return showAlert("addUserError", "Valid email is required");
  if (!payload.deptId)
    return showAlert("addUserError", "Department is required");
  if (!payload.role) return showAlert("addUserError", "Role is required");
  if (role === "Student" && !payload.batch)
    return showAlert("addUserError", "Batch Year is required");
  if (role === "Student" && !payload.degree)
    return showAlert("addUserError", "Degree is required");
  if (role === "Instructor" && !payload.designation)
    return showAlert("addUserError", "Designation is required");
...
function openEditUserModal(userId) {
  ...
  document
    .querySelectorAll(".edit_studentRow, .edit_instructorRow, .edit_adminRow")
  ...
  $.ajax({
    ...
    success: function (res) {
      ...
      if (u.role === "Student") {
        ...
        document
          .querySelectorAll(".edit_studentRow")
          .forEach((r) => (r.style.display = ""));
        ...
        document
          .querySelectorAll(".edit_instructorRow")
          .forEach((r) => (r.style.display = ""));
        document.getElementById("edit_cur_designation").value =
          u.designation || "";
        document.getElementById("edit_cur_office").value = u.officeRoom || "";
        document.getElementById("edit_cur_spec").value = u.specialization || "";
        document.getElementById("edit_new_designation").value =
          u.designation || "";
        ...
        document
          .querySelectorAll(".edit_adminRow")
          .forEach((r) => (r.style.display = ""));
        document.getElementById("edit_cur_adminDesignation").value =
          u.designation || "";
        document.getElementById("edit_cur_adminOffice").value =
          u.officeRoom || "";
        document.getElementById("edit_new_adminDesignation").value =
          u.designation || "";
        document.getElementById("edit_new_adminOffice").value =
          u.officeRoom || "";
      }
...
function submitEditUser() {
  ...
  if (role === "Student") {
    ...
    payload.officeRoom = document
      .getElementById("edit_new_office")
      .value.trim();
    payload.specialization = document
      .getElementById("edit_new_spec")
      .value.trim();
  } else if (role === "Admin") {
    payload.designation = document
      .getElementById("edit_new_adminDesignation")
      .value.trim();
    payload.officeRoom = document
      .getElementById("edit_new_adminOffice")
      .value.trim();
...
function openDeleteUserModal(userId, fullName) {
  ...
  document.getElementById("delete_userName").textContent =
    `${fullName} (${userId})`;
    ...
    success: function (res) {
      if (res.success) {
        ...
        const row = document.querySelector(
          `#usersTable tr[data-user-id="${userId}"]`,
        );
        if (row) {
  ...
  $.ajax({
    ...
    success: function (res) {
      if (res.success) {
        ...
        setTimeout(() => {
          bootstrap.Modal.getInstance(
            document.getElementById("assignCourseModal"),
          ).hide();
          location.reload();
        }, 1000);
      } else {
